...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  # +++处理多标签
  label_id = predicate_label_to_id(example.label.strip().split(" "), label_map)  # +++
  if ex_index < 5:
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["label_ids"] = create_int_feature(feature.label_id)  # +++原版有中括号
...
def file_based_input_fn_builder(input_file, seq_length, label_length, is_training,
  ...
  name_to_features = {
      ...
      "label_ids": tf.FixedLenFeature([label_length], tf.int64),  # +++对应上面的
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    probabilities = tf.sigmoid(logits)  # +++损失函数改为多标签
